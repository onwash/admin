...
import { TextField, Divider, MenuItem, Dialog, Button } from '@material-ui/core';
...
export  default function(){
  ...
  const [createWashCoordinates, { data }] = useMutation(CREATE_WASH_COORDINATES)
  
  const [createWashDescription] = useMutation(CREATE_WASH_DESC, {
    update(cache, { data: { createWashDescription } }){
      const { getAllWashDescriptions }:any = cache.readQuery({ query: GET_WASH })
      console.log(getAllWashDescriptions)
      cache.writeQuery({
        query: GET_WASH,
        data: { getAllWashDescriptions: getAllWashDescriptions.concat([createWashDescription]) },
      })
    }
  })
    ...
    >
      {props => (
        ...
        <DialogContent style ={{ padding: '0 24px'}}>
          <DialogContentText id="alert-dialog-description" style = {{marginBottom : 0}} >

          <div style= {{ display: 'flex', flexDirection: 'row'}}>

            <TextField
              style = {{ marginTop: '1vh', marginBottom: '1vh', marginRight: '1vw'}}
              size = 'small'
              label='Широта'
              variant="outlined"
       ...
       { newWashId === null ? 
         ...
         <div>
            <Formik
              initialValues={{ coordinatesId: newWashId, washname: "", adress: '', uptime: '', postCount: 0, region:'' }}
              onSubmit={(values, actions) => {
