...
const S: FC<ISProps> = (props)=>{
  ...
  const handlePlacemark = async (id: string) =>{
    getAllWashDescriptions({variables:{
      coordinatesId: id
    }})
  }
  React.useEffect(()=>{
  ...
  React.useEffect(()=>{
    if(data){
      setprepareCoordinates(data.getWashCoordinates.map((i:any)=> [i.latitude , i.longitude, i.id]))
    }
  }, [data])
  console.log(wash)
  if(error) return <Err />
...
{
  prepareCoordinates.map((i:any)=> <Placemark key={i[2]} geometry={i} onClick = {()=>handlePlacemark(i[2])}/>) //третий элемент в массиве это id
}
